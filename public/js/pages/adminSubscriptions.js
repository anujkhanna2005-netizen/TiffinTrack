  ...
  try {
    ...
    showContent(`
      <div class="page-header" style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px">
        <div>
          <h1>📋 Subscription Approvals & Directory</h1>
          <p>Review student subscription applications, execute platform-wide admin approvals, and monitor diner meals</p>
        </div>
        <div>
      ...
      <div class="stats-grid" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); margin-bottom:24px">
        <div class="stat-card">
          <div class="stat-value" style="color:${pendingSubs.length > 0 ? '#ea580c' : 'var(--color-text)'}">
            ${pendingSubs.length}
          </div>
          <div class="stat-label">Pending Approval Requests</div>
        </div>
        <div class="stat-card">
          <div class="stat-value" style="color:var(--color-success)">${activeSubs.length}</div>
          <div class="stat-label">Active Diners</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${formatINR(totalRevenue)}</div>
          <div class="stat-label">Active Monthly Value</div>
        </div>
        <div class="stat-card">
      ...
      <div class="card" style="margin-bottom:24px; border-left:4px solid var(--color-primary); background:#fcfdfd">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          <div class="card-title" style="margin-bottom:0">
            <span class="icon">⏳</span> Pending Subscription Requests (${pendingSubs.length})
          </div>
          <span class="badge" style="background:#dbeafe;color:#1e40af;font-weight:700">Platform Tie-up Authority</span>
        </div>

        ${pendingSubs.length === 0 ? `
          <div style="text-align:center;padding:32px;color:var(--color-text-muted)">
            <div style="font-size:2rem;margin-bottom:8px">🎉</div>
            <strong>All subscription requests have been reviewed!</strong>
                  ...
                  <tr id="admin-sub-row-${s.sub_id}">
                    ...
                    <td>
                      <div style="display:flex;gap:6px">
                        <button class="btn btn-primary btn-sm" style="background:var(--color-success);border-color:var(--color-success);padding:5px 12px;font-size:0.8rem;font-weight:600"
      ...
      <div class="card">
        <div class="card-title"><span class="icon">📜</span> Platform Subscriptions Directory (${activeSubs.length + pastSubs.length})</div>
        <div class="table-wrapper">
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              ${[...activeSubs, ...pastSubs].map(s => `
                <tr>
                  ...
                  <td><span class="badge" style="background:#e0e7ff;color:#3730a3">COD</span></td>
                  <td>${subscriptionStatusBadge(s.status)}</td>
                  <td style="font-size:0.8rem;color:var(--color-text-muted)">
                    ${s.approved_by ? `<span class="badge" style="background:#f1f5f9;color:#334155">${s.approved_by.toUpperCase()}</span>` : '—'}
                  </td>
                  <td style="font-size:0.8rem;color:var(--color-text-muted)">
